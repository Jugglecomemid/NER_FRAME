...
class BertCrf(nn.Module):
    def __init__(self, bert_model, labels_idx_dict, batch_size, dropout, hidden_size, device):
        ...
        self.full_connected_layer = nn.Linear(self.hidden_size, self.labels_num)
    ...
    def _get_bert_features(self, input_ids, segment_ids, input_mask):
        ...
        bert_features = self.full_connected_layer(last_hidden_state)
    ...
    def negate_log_likelihood(self, input_ids, segment_ids, input_mask, label_ids):
        ...
        计算损失函数
        """
        bert_features = self._get_bert_features(input_ids, segment_ids, input_mask)
        forward_score = self._forward_alg(bert_features)
        sentence_tags_score = self._get_sentence_tags_score(bert_features, label_ids)
    ...
    def forward(self, input_ids, segment_ids, input_mask):
        ...
        """
        bert_features = self._get_bert_features(input_ids, segment_ids, input_mask)
        score, label_seq_ids = self._viterbi_decode(bert_features)
...
class BertCrfNer(nn.Module):
    ...
    def get_model(self):
        ...
        checkpoint_path = os.path.join(self.common_config.checkpoint_dir, self.model_config.bert_crf_model_name)
        if self.model_config.load_checkpoint and os.path.exists(checkpoint_path):
            ...
            print("Loaded pretrain bert crf ner model, previous acc: {}, previous f1_score : {}".format(self.prev_acc_score, self.prev_f1_score))
        return model